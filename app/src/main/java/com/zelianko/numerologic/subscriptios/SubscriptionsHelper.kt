package com.zelianko.numerologic.subscriptios
...
data class SubscriptionsHelper(private val context: Context, val productId: String){
    ...
    private val _textPrice = MutableStateFlow("")
    val textPrice = _textPrice.asStateFlow()
    ...
    fun queryProduct(productId: String) {
        ...
        billingClient.queryProductDetailsAsync(
            ...
            if (productDetailsList.isNotEmpty()) {
                ...
                _textPrice.value = productDetails.subscriptionOfferDetails?.get(0)?.pricingPhases?.pricingPhaseList?.get(0)?.formattedPrice.toString()
                Log.i(PURCHASE_STATUS,"Purchase Product: ${productDetails.name}")
                Log.i(PURCHASE_STATUS,"Price Product: ${productDetails.name}")
            } else {
    ...
    private fun completePurchase(item: Purchase) {
        ...
        if (purchase.purchaseState == Purchase.PurchaseState.PURCHASED) {
            _purchaseDone.value = false
            _purchaseStatus.value = "DONE"
            Log.i(PURCHASE_STATUS, "DONE")
            GlobalScope.launch {
                handlePurchase()
            }
        }
...
//                    .setPurchaseToken(purchase.purchaseToken).build()
                ...
                _purchaseAcknowledged.value = "ACKNOWLEDGE CODE:: ${ackPurchaseResult.billingResult.responseCode}"
            }
        }
    }
